...
- **早期終了対策**: iOS Safari は `continuous` を無視して1発話で認識を終えることがある。`speechRecognizer` は「最後の認識結果から約1秒 無音が続いたら確定」というルールを自前で持ち、プラットフォームが早期に `onend` を出しても予算内（最大リスタート回数 / 総時間）で `start()` を呼び直して聞き取りを継続する
