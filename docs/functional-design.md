...
| allDay | boolean | 終日予定なら `true`。`true` のとき時刻部分は無視し、日付のみで扱う。複数日にまたがる予定も `allDay=true` で表し、`endAt` は「最終日の翌日 0:00」（排他的終端）とする |
...
| 画面 | 役割 | 主な操作 |
|---|---|---|
| 日別ビュー（ホーム） | 起動時に表示。選択日の予定を時系列で一覧。フッターに赤（すべての予定）・黄（書き出し）・青（音声）の丸ボタンと ＋（手入力） | 日の移動（矢印 or 左右スワイプ：左＝翌日 / 右＝前日）、日付タップで月別ビュー（日付選択）、「今日」ボタンで当日へ、予定タップで編集、音声入力、手入力、書き出し、メニュー、すべての予定 |
| 月別ビュー | 1か月のカレンダー。予定のある日にマーク | 日タップで日別ビューへ、月の移動 |
| すべての予定（一覧） | 全予定を日付ごとに箇条書きで時系列表示（日別ビュー左下の赤い丸ボタンから開く）。複数日の終日予定は期間中の各日に表示する | 項目タップで予定確認フォーム（編集）、閉じる |
...
| ◯月◯日から◯日 / ◯月◯日〜◯月◯日 / ◯日から◯日 | **日付範囲**。複数日にまたがる終日予定として登録する（開始日〜最終日）。区切りは「から」「〜」「～」「-」等 |
...
| 予定ストア | Event の CRUD、端末内ストレージ（IndexedDB）への読み書き、保存失敗時に既存データを保護（`architecture.md` で方式確定）。範囲取得は「期間の重なり」で判定し、複数日予定を各日で拾えるようにする |
